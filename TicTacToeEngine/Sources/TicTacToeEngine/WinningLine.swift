...
@frozen
public enum Grid3WinningLine: WinningLineProtocol {
    case horizontal(Vertical3Position)
    case vertical(Horizontal3Position)
    case diagonal(isBackslash: Bool) // forwardslash: "/", backslash: "\"

    public static let allCases: Set<Grid3WinningLine> = {
        let horizontalLines = Vertical3Position.allCases.map { Self.horizontal($0) }
        let verticalLines = Horizontal3Position.allCases.map { Self.vertical($0) }
...
@frozen
public enum Cube4WinningLine: WinningLineProtocol {
    case horizontal(Vertical4Position, Depth4Position)
    case vertical(Horizontal4Position, Depth4Position)
    case depth(Horizontal4Position, Vertical4Position)
    case zDiagonal(Depth4Position, isBackslash: Bool) // forwardslash: "/", backslash: "\"
    case yDiagonal(Vertical4Position, isBackslash: Bool)
    case xDiagonal(Horizontal4Position, isBackslash: Bool)
    case crossDiagonal(isFront: Bool, isBackslash: Bool)

    public static let allCases: Set<Cube4WinningLine> = {
        let horizontalLines: [Cube4WinningLine] = Vertical4Position.allCases
            .flatMap { yPos in Depth4Position.allCases.map { zPos in .horizontal(yPos, zPos) } }
        let verticalLines: [Cube4WinningLine] = Horizontal4Position.allCases
            .flatMap { xPos in Depth4Position.allCases.map { zPos in .vertical(xPos, zPos) } }
        let depthLines: [Cube4WinningLine] = Horizontal4Position.allCases
            .flatMap { xPos in Vertical4Position.allCases.map { yPos in .depth(xPos, yPos) } }
        let zDiagnolLines: [Cube4WinningLine] = Depth4Position.allCases
            .flatMap { zPos in Bool.allCases.map { isBackslash in .zDiagonal(zPos, isBackslash: isBackslash) } }
        let yDiagnolLines: [Cube4WinningLine] = Vertical4Position.allCases
            .flatMap { yPos in Bool.allCases.map { isBackslash in .yDiagonal(yPos, isBackslash: isBackslash) } }
        let xDiagnolLines: [Cube4WinningLine] = Horizontal4Position.allCases
            .flatMap { xPos in Bool.allCases.map { isBackslash in .xDiagonal(xPos, isBackslash: isBackslash) } }
        let crossDiagonalLines: [Cube4WinningLine] = Bool.allCases
